...
function MediaDialog({ multiple, onSelect, onClose }: Props & { onClose: () => void }) {
	...
	useEffect(() => {
		...
		fetch('/api/upload', { signal: controller.signal, cache: 'no-store' })
			.then(async (response) => {
	...
	const filtered = photos.filter((photo) => (!folder || photo.folder === folder) && `${photo.name} ${photo.folder}`.toLowerCase().includes(search.toLowerCase()));
	const folders = [...new Set(photos.map((photo) => photo.folder).filter(Boolean))].sort();
	...
	return createPortal(<dialog className='media-dialog' ref={dialog} aria-labelledby={titleId} onCancel={(event) => { if (busy) event.preventDefault(); }} onClose={onClose}>
		...
		<div className='media-dialog-body'>
			{tab === 'photos' ? <>
				<div className='media-filters'><input aria-label='Search uploaded photos' type='search' placeholder='Search photos…' value={search} onChange={(event) => { setSearch(event.target.value); setLimit(40); }} /><select aria-label='Photo folder' value={folder} onChange={(event) => { setFolder(event.target.value); setLimit(40); }}><option value=''>All folders</option>{folders.map((name) => <option key={name}>{name}</option>)}</select><button type='button' className='secondary-button' onClick={() => { setLoading(true); setError(''); setRefresh((value) => value + 1); }}>Refresh</button></div>
				{loading ? <p role='status'>Loading uploaded photos…</p> : <><p>{filtered.length} photos · {selected.length} selected</p><div className='media-grid'>{filtered.slice(0, limit).map((photo) => <button type='button' key={photo.id} className='media-photo' aria-label={`Select ${photo.name}`} aria-pressed={selected.includes(photo.url)} onClick={() => setSelected((current) => multiple ? current.includes(photo.url) ? current.filter((item) => item !== photo.url) : [...current, photo.url] : [photo.url])}><img loading='lazy' src={photo.url} alt={photo.name} /><span title={photo.name}>{photo.name}</span></button>)}</div>{!filtered.length ? <p>No photos found. Upload an image or change your filters.</p> : null}{filtered.length > limit ? <button type='button' className='secondary-button' onClick={() => setLimit((count) => count + 40)}>Load more photos</button> : null}</>}
